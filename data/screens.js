...
(() => {
    ...
    // --- HELPER: Play NIBP Sound ---
    const playNibpSound = () => {
        // Simple synthetic "inflation" sound using Web Audio API if available
    ...
    const SetupScreen = ({ onGenerate, savedState, onResume }) => {
        ...
        const handleGenerate = () => {
             ...
             if (pool.length === 0) {
                 alert("No scenarios found matching this combination.");
                 return;
             }
                        ...
                        <div className="space-y-2">
                             ...
                             {customScenarios.map((s, i) => (
                                 <div key={i} className="flex justify-between items-center bg-slate-700/50 p-3 rounded border border-slate-600">
                                     <div><div className="font-bold text-white">{s.title}</div><div className="text-xs text-slate-400">{s.category}</div></div>
            ...
            <div className="flex flex-col items-center justify-center h-full bg-slate-900 text-white p-4">
                <div className="w-full max-w-md space-y-6 text-center">
                    ...
                    <input 
                        ...
                        className="w-full bg-slate-800 border-2 border-slate-600 rounded-lg p-4 text-center text-3xl font-mono tracking-widest uppercase text-white focus:border-sky-500 outline-none"
                        maxLength={4}
                    />
            ...
            <div className="bg-slate-800 p-6 rounded-lg border-l-4 border-sky-500 shadow-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-4">
                         ...
                         <div className="bg-slate-900/50 p-3 rounded border border-slate-600"><h4 className="text-sm font-bold text-purple-400 uppercase mb-2">Debrief Points</h4><ul className="list-disc pl-4 text-sm text-slate-300">{scenario.instructorBrief.debriefPoints && scenario.instructorBrief.debriefPoints.map((l, i) => <li key={i}>{l}</li>)}</ul></div>
                    </div>
                </div>
            </div>
    ...
    // --- SCREEN 4: LIVE SIM CONTROLLER (REBUILT) ---
    const LiveSimScreen = ({ sim, onFinish, onBack }) => {
        ...
        const getInterventionsByCat = (cat) => {
            if (cat === 'Handover') return []; // Special tab
            let keys = [];
        ...
        // SBAR GENERATOR
        const generateSBAR = () => {
            return `S: ${scenario.title}. ${scenario.ageRange} patient.\n` +
                   `B: ${scenario.patientProfileTemplate.replace('{age}', scenario.patientAge)}.\n` +
                   `A: Current Stats: HR ${vitals.hr}, BP ${vitals.bpSys}/${vitals.bpDia}, SpO2 ${vitals.spO2}%. \n   Interventions: ${Array.from(activeInterventions).join(', ') || 'None'}.\n` +
                   `R: Request urgent review / transfer to ICU.`;
        };
            ...
            <div className={`h-full overflow-hidden flex flex-col p-2 bg-slate-900 relative ${flash === 'red' ? 'flash-red' : (flash === 'green' ? 'flash-green' : '')}`}>
                ...
                <div className="flex justify-between items-center bg-slate-800 p-2 rounded mb-2 border border-slate-700">
                    <div className="flex gap-2">
                        ...
                        <Button variant="outline" onClick={() => window.open(window.location.href.split('?')[0] + '?mode=monitor&session=' + (new URLSearchParams(window.location.search).get('session') || ''), '_blank', 'popup=yes')} className="h-8 px-2 text-xs"><Lucide icon="external-link"/> Open Monitor</Button>
                    </div>
                    <div className="flex items-center gap-3">
                        <div className="text-right hidden md:block">
                            <div className="text-[10px] text-slate-400 uppercase">Sim Time</div>
                            <div className="font-mono text-xl font-bold text-emerald-400 leading-none">{formatTime(time)}</div>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-2 overflow-hidden min-h-0">
                    ...
                    {/* --- LEFT COL: VITALS CONTROL & MONITOR --- */}
                    <div className="lg:col-span-4 flex flex-col gap-2 overflow-y-auto">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                             <div className="bg-slate-800 p-2 rounded border border-slate-600">
                                 ...
                                 <div className="flex gap-1">
                                     ...
                                     <Button onClick={() => sim.dispatch({type: 'TOGGLE_NIBP_MODE'})} variant={nibp.mode === 'auto' ? "success" : "secondary"} className="h-8 text-[10px] flex-1">
                                         {nibp.mode === 'auto' ? `Auto (${Math.ceil(nibp.timer/60)}m)` : 'Auto Off'}
                                     </Button>
                                 </div>
                             </div>
                             <div className="bg-slate-800 p-2 rounded border border-slate-600">
                        ...
                        <div className="bg-slate-800 p-2 rounded border border-slate-600 relative z-10">
                             ...
                             <Button onClick={() => setExpandRhythm(!expandRhythm)} variant="secondary" className="w-full h-8 text-xs justify-between">
                                 {rhythm} <Lucide icon="chevron-down" className="w-3 h-3"/>
                             </Button>
                             {expandRhythm && (
                                 <div className="absolute top-full left-0 w-full bg-slate-800 border border-slate-500 rounded shadow-xl max-h-60 overflow-y-auto mt-1">
                                     {['Sinus Rhythm', 'Sinus Tachycardia', 'Sinus Bradycardia', 'AF', 'SVT', 'VT', 'VF', 'Asystole', 'PEA', 'STEMI', '1st Deg Block', '3rd Deg Block'].map(r => (
                                         <button key={r} onClick={() => {sim.dispatch({type: 'UPDATE_RHYTHM', payload: r}); setExpandRhythm(false);}} className="block w-full text-left px-3 py-2 text-xs text-white hover:bg-sky-600 border-b border-slate-700">{r}</button>
                                     ))}
                                 </div>
                             )}
                             <div className="flex gap-1 mt-2">
                                <Button onClick={triggerArrest} variant="danger" className="flex-1 h-8 text-[10px]">Arrest</Button>
                                <Button onClick={triggerROSC} variant="success" className="flex-1 h-8 text-[10px]">ROSC</Button>
                             </div>
                        </div>
                    </div>

                    {/* --- RIGHT COL: INTERVENTIONS DRAWER --- */}
                    <div className="lg:col-span-8 flex flex-col bg-slate-800 rounded border border-slate-700 overflow-hidden">
                        ...
                        <div className="flex overflow-x-auto bg-slate-900 border-b border-slate-700 no-scrollbar">
                            {['Common', 'Airway', 'Breathing', 'Circulation', 'Drugs', 'Procedures', 'Handover'].map(cat => (
                                <button key={cat} onClick={() => setActiveTab(cat)} className={`px-4 py-3 text-xs font-bold uppercase tracking-wider transition-colors whitespace-nowrap ${activeTab === cat ? 'bg-slate-800 text-sky-400 border-t-2 border-sky-400' : 'text-slate-500 hover:text-slate-300'}`}>
                                    {cat}
                                </button>
                            ))}
                        </div>
                        ...
                        <div className="flex-1 p-2 overflow-y-auto bg-slate-800 relative">
                            {/* Handover Tab Special */}
                            {activeTab === 'Handover' ? (
                                <div className="space-y-4 p-2">
                                    <div className="bg-slate-900 p-4 rounded border border-slate-600 font-mono text-sm text-green-400 whitespace-pre-wrap">
                                        {generateSBAR()}
                                    </div>
                                    <div className="flex gap-2">
                                        <Button onClick={() => speak(generateSBAR())} variant="secondary">Read Aloud (TTS)</Button>
                                        <Button onClick={() => navigator.clipboard.writeText(generateSBAR())} variant="outline">Copy to Clipboard</Button>
                                    </div>
                                </div>
                            ) : (
                                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                                    {getInterventionsByCat(activeTab).map(key => {
                                        ...
                                        // Visual State
                                        let btnClass = isActive ? "border-l-4 border-l-emerald-500" : `opacity-90 hover:opacity-100 ${getCatColor(activeTab)}`;
                                        if (isActive) btnClass = "bg-emerald-900/40 border border-emerald-500 text-emerald-100";
                                        ...
                                        return (
                                            <button 
                                                ...
                                                disabled={!isRunning}
                                                className={`relative h-16 p-2 rounded text-left transition-all active:scale-95 flex flex-col justify-between overflow-hidden shadow-sm ${btnClass}`}
                                            >
                                                ...
                                                <div className="flex justify-between items-end w-full">
                                                    ...
                                                    {count > 0 && action.type !== 'continuous' && <span className="bg-white text-black text-[9px] font-bold px-1.5 rounded-full">x{count}</span>}
                                                </div>
                                                {/* Timer Bar */}
                                                {activeDurations[key] && (
                                                    <div className="absolute bottom-0 left-0 h-1 bg-emerald-400 transition-all duration-1000" style={{width: `${Math.max(0, 100 - ((time - activeDurations[key].startTime)/activeDurations[key].duration*100))}%`}}></div>
                                                )}
                                            </button>
                                        );
                                    })}
                        ...
                        <div className="bg-slate-900 p-2 border-t border-slate-700 flex gap-2">
                             <input type="text" className="bg-slate-800 border border-slate-600 rounded px-3 text-xs flex-1 text-white" placeholder="Search Interventions or Type Log..." value={customLog} onChange={e=>setCustomLog(e.target.value)} onKeyDown={e => e.key === 'Enter' && (addLogEntry(customLog, 'manual') || setCustomLog(""))} />
                             <Button onClick={() => {sim.dispatch({type: 'TRIGGER_IMPROVE'}); addLogEntry("Instructor: Patient Improving", "success")}} className="h-8 text-xs px-2 bg-emerald-900 border border-emerald-500 text-emerald-100">Improve</Button>
                             <Button onClick={() => {sim.dispatch({type: 'TRIGGER_DETERIORATE'}); addLogEntry("Instructor: Patient Deteriorating", "danger")}} className="h-8 text-xs px-2 bg-red-900 border border-red-500 text-red-100">Worsen</Button>
                        </div>
                    </div>
                </div>
                        ...
                        <div className="bg-slate-800 p-6 rounded-lg border border-slate-600 w-full max-w-sm">
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex gap-2 mt-4">
                                    <Button onClick={()=>setShowTrends(false)} variant="secondary" className="flex-1">Cancel</Button>
                                    <Button onClick={()=>{startTrend({hr: trendParams.hr, bpSys: trendParams.bpSys}, trendParams.duration); setShowTrends(false);}} variant="primary" className="flex-1">Start Trend</Button>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    const MonitorScreen = ({ sim }) => {
        ...
        // NIBP Sound Trigger
        useEffect(() => {
            if (nibp.lastTaken && audioEnabled) playNibpSound();
        }, [nibp.lastTaken]);
                ...
                {!audioEnabled && (
                    <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm" onClick={() => { enableAudio(); setAudioEnabled(true); }}>
                        <div className="bg-slate-800 border border-sky-500 p-6 rounded-lg shadow-2xl animate-bounce cursor-pointer text-center">
                            <Lucide icon="volume-2" className="w-12 h-12 text-sky-400 mx-auto mb-2"/>
                            <h2 className="text-xl font-bold text-white">Tap to Enable Sound</h2>
                        </div>
                    </div>
                )}
    ...
    const DebriefScreen = ({ sim, onRestart }) => {
        ...
        useEffect(() => {
            ...
            if (window.myChart) window.myChart.destroy(); // Cleanup previous chart
    ...
    const LiveSimContainer = ({ sim, view, setView, resumeData, onRestart }) => {
        ...
        useEffect(() => {
            if (view === 'resume' && resumeData) { sim.dispatch({ type: 'RESTORE_SESSION', payload: resumeData }); } 
            else if (!scenario) { setView('setup'); }
        }, []);
