...
(() => {
    ...
    const SetupScreen = ({ onGenerate, savedState, onResume, sessionID, onJoinClick }) => {
        ...
        // Update preview when scenario is selected
        useEffect(() => {
            if (selectedScen) {
                // Generate temporary details for preview
                                ...
                                {selectedScen && previewDetails ? (
                                    <div className="animate-fadeIn space-y-4">
                                        ...
                                        <div className="text-xs space-y-2 text-slate-300">
                                            ...
                                            <div className="p-2 bg-slate-800 rounded border border-slate-600">
                                                <strong className="text-emerald-400 block mb-1">Learning Outcomes</strong>
                                                <ul className="list-disc pl-4">{selectedScen.instructorBrief.learningObjectives.map(l => <li key={l}>{l}</li>)}</ul>
                                            </div>
                                            {selectedScen.learningLinks && (
                                                <div className="p-2 bg-slate-800 rounded border border-slate-600">
                                                    <strong className="text-purple-400 block mb-1">Guidelines</strong>
                                                    <div className="flex flex-col gap-1">
                                                        {selectedScen.learningLinks.map((l, i) => (
                                                            <a key={i} href={l.url} target="_blank" className="text-sky-300 hover:underline flex items-center gap-1"><Lucide icon="link" className="w-3 h-3"/> {l.label}</a>
                                                        ))}
                                                    </div>
                                                </div>
                                            )}
            ...
            <div className={`h-full overflow-hidden flex flex-col p-2 bg-slate-900 relative ${flash === 'red' ? 'flash-red' : (flash === 'green' ? 'flash-green' : '')}`}>
                ...
                <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-2 overflow-hidden min-h-0">
                    ...
                    <div className="lg:col-span-8 flex flex-col bg-slate-800 rounded border border-slate-700 overflow-hidden">
                        ...
                        <div className="flex-1 p-2 overflow-y-auto bg-slate-800 relative">
                            ...
                            {activeTab === 'Common' && (
                                <div className="mb-2 p-2 bg-slate-900 rounded border border-slate-700 flex items-center justify-between">
                                    <span className="text-[10px] font-bold uppercase text-slate-500">Audio Output Source</span>
                                    <div className="flex gap-1">
                                        {['monitor', 'controller', 'both'].map(o => (<button key={o} onClick={() => sim.dispatch({type: 'SET_AUDIO_OUTPUT', payload: o})} className={`px-2 py-1 text-[9px] uppercase font-bold rounded ${audioOutput === o ? 'bg-sky-600 text-white' : 'bg-slate-700 text-slate-400'}`}>{o}</button>))}
                                    </div>
                                </div>
                            )}
                ...
                {showTrends && (
                    <div className="absolute inset-0 z-50 bg-black/80 flex items-center justify-center p-4">
                        <div className="bg-slate-800 p-6 rounded-lg border border-slate-600 w-full max-w-sm"><h3 className="text-lg font-bold text-white mb-4">Set Vitals Trend</h3><div className="space-y-3"><div><label className="text-xs text-slate-400">Target HR</label><input type="number" value={trendParams.hr} onChange={e=>setTrendParams({...trendParams, hr: parseInt(e.target.value)})} className="w-full bg-slate-900 border border-slate-600 rounded p-2 text-white"/></div><div><label className="text-xs text-slate-400">Target BP Sys</label><input type="number" value={trendParams.bpSys} onChange={e=>setTrendParams({...trendParams, bpSys: parseInt(e.target.value)})} className="w-full bg-slate-900 border border-slate-600 rounded p-2 text-white"/></div><div><label className="text-xs text-slate-400">Duration (seconds)</label><input type="number" value={trendParams.duration} onChange={e=>setTrendParams({...trendParams, duration: parseInt(e.target.value)})} className="w-full bg-slate-900 border border-slate-600 rounded p-2 text-white"/></div><div className="flex gap-2 mt-4"><Button onClick={()=>setShowTrends(false)} variant="secondary" className="flex-1">Cancel</Button><Button onClick={()=>{startTrend({hr: trendParams.hr, bpSys: trendParams.bpSys}, trendParams.duration); setShowTrends(false);}} variant="primary" className="flex-1">Start Trend</Button></div></div></div>
                    </div>
                )}
            </div>
        );
